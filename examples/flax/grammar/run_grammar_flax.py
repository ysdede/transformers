...
class DataTrainingArguments:
    ...
    source_column: Optional[str] = field(
        default="input",
        metadata={"help": "The name of the column in the datasets containing the corrupt (for grammar)."},
    )
    target_column: Optional[str] = field(
        default="target",
        metadata={"help": "The name of the column in the datasets containing the correct (for grammar)."},
    )
...
def main():
    ...
    send_example_telemetry("run_grammar", model_args, data_args, framework="flax")
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(examples[source_column])):
            ...
            if data_args.min_length and (len(source_text) < data_args.min_length or len(target_text) < data_args.min_length):
                continue
            if source_text and target_text:
                #  add 'grammar:' prefix to source texts if not exist.
        ...
        # logger.info(inputs[0], targets[0])
        # logger.info(len(inputs), len(inputs))
    ...
    for epoch in epochs:
        ...
        # Save metrics
        if has_tensorboard and jax.process_index() == 0:
            cur_step = (epoch + 1) * (len(train_dataset) // train_batch_size)
